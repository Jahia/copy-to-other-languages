...
export class Picker extends BaseComponent {
    ...
    navigateTo(accordion: AccordionItem, path: string) {
        ...
        this.wait()
        accordion
            .getTreeItem(selectPath)
            .children('div.moonstone-treeView_item')
    ...
    verifyResultsLength(length: number) {
        this.get()
            .find('[data-sel-role="table-pagination-total-rows"]')
            .should('be.visible')
            .and('contain', `of ${length}`)
    }
    ...
    uploadFile(pathToFixture: string) {
        this.get().find('table[data-cm-role="table-content-list"]').selectFile(pathToFixture, {
            action: 'drag-drop',
