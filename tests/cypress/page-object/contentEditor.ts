...
export class ContentEditor extends BasePage {
    ...
    getLanguageSwitcher(): LanguageSwitcher {
        if (!this.languageSwitcher) {
            this.languageSwitcher = getComponentBySelector(
                LanguageSwitcher,
                '#contenteditor-dialog-title [data-cm-role="language-switcher"]',
            )
        }
    ...
    getLanguageSwitcherAdvancedMode(): LanguageSwitcher {
        if (!this.languageSwitcher) {
            this.languageSwitcher = getComponentBySelector(
                LanguageSwitcher,
                '.moonstone-dropdown_container [data-cm-role="language-switcher"]',
            )
        }
    ...
    toggleOption(optionType: string, optionFieldName: string) {
        cy.get(`span[data-sel-role-dynamic-fieldset="${optionType}"]`)
    ...
    checkButtonStatus(role: string, enabled: boolean) {
        getComponentByRole(Button, role)
            .should('be.visible')
